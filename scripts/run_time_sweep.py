...
def main() -> int:
    ...
    for spec in run_specs:
        ...
        shell_cmd = " ".join(shlex.quote(part) for part in command_parts)
        (run_dir / "command.sh").write_text(shell_cmd + "\n", encoding="utf-8")

        print(f"[{spec.seconds:g}s -> render-time={spec.seconds:g}] {run_dir}")

        with (run_dir / "console.log").open("w", encoding="utf-8") as log_file:
    ...
    if failure_records:
        ...
        for record in failure_records:
            print(
                ...
                f"exit code {record.returncode} -> {record.run_dir}"
            )
